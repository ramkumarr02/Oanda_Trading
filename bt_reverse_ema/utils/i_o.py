...
def plot_graph(data):
    ...
    ax2.plot(data['df']["ema_diff"], c='green', label = 'sema_angle', linestyle = linestyle)
    ...
    plt.grid()
    plt.show()    
    data['chart_name'] = f'{data["input_file_name"].split(".")[0]}.png'
...
def create_report(data):
    ...
    # data['file_name'] = f'data/products/{data["product"]}/{data["start_date"].year}-({data["start_date"].month}-{data["end_date"].month})-({data["start_date"].day}-{data["end_date"].day})-{data["start_ts"]}.csv'
    data['file_name'] = f'{data["start_date"].year}-({data["start_date"].month}-{data["end_date"].month})-({data["start_date"].day}-{data["end_date"].day})-{data["start_ts"]}.csv'
